...
const LIST_REGEX = /^(<!--\s*docheat:(\w+)\s*-->)(?:[^]*?(^<!--\s*endcheat\s*-->))?/mg;
...
export async function updateDocumentationFile(
    ...
    content = content.replace(LIST_REGEX, (text: string, start: string, type: string, end: string) => {
        let parts = [start];

        switch (type) {
            case 'functions':
                let functionsText = exports
                    .functions
