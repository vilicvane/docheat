...
export default function(
  ...
  let {fileName, lineNumber, name, overloads, documentation} = entry;

  let headingSharps = Array(options.level + 1).join('#');

  let overloadsPlusText = overloads ? `<sup>+${overloads.length}</sup>` : '';

  fileName = Path.relative(Path.dirname(options.fileName), fileName).replace(
    /\\/g,
    '/',
  );
  ...
  let text = `\
${headingSharps} [[+]](${url}#L${lineNumber}) \`${name}${signatureToString(
    entry,
  )}\`${overloadsPlusText}\n\n`;
...
function signatureToString(entry: SignatureEntry): string {
  let {typeParameters, parameters, returnType} = entry;
